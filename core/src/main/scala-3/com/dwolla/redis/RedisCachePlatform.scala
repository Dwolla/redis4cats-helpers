...
private[redis] trait RedisCachePlatform extends RedisCachePlatformLowPriority:
  implicit def derivedAspect[K, V, Dom[_], Cod[_]](using Dom[K],
                                                   ...
                                                   Cod[Unit],
                                                   Cod[Option[V]],
                                                  ): Aspect[[F[_]] =>> RedisCache[F, K, V], Dom, Cod] =
    new Aspect[[F[_]] =>> RedisCache[F, K, V], Dom, Cod]:
      override def weave[F[_]](af: RedisCache[F, K, V]): RedisCache[Aspect.Weave[F, Dom, Cod, *], K, V] =
        new RedisCache[Aspect.Weave[F, Dom, Cod, *], K, V]:
          override def get(key: K): Aspect.Weave[F, Dom, Cod, Option[V]] =
            Aspect.Weave("RedisCache", List(List(Aspect.Advice.byValue("key", key))), Aspect.Advice("get", af.get(key)))

          override def set(key: K, value: V, setArgs: effects.SetArgs): Aspect.Weave[F, Dom, Cod, Boolean] =
          ...
          override def del(key: K): Aspect.Weave[F, Dom, Cod, Long] =
          ...
          override def updateExisting(key: K, existing: V)
                                     (newValue: V, expiresIn: FiniteDuration): Aspect.Weave[F, Dom, Cod, Unit] =
          ...
          override def deleteExisting(key: K, existing: V): Aspect.Weave[F, Dom, Cod, Unit] =
            Aspect.Weave("RedisCache", List(List(
              Aspect.Advice.byValue("key", key),
              Aspect.Advice.byValue("existing", existing),
...
private[redis] class RedisCacheMapKd[F[_], G[_], K, V](af: RedisCache[F, K, V],
